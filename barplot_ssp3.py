...
east_obs=ts_obs_a.sel(lat=slice(-5,5),lon=slice(180,280)).mean('lat').mean('lon').mean('lev')
west_obs=ts_obs_a.sel(lat=slice(-5,5),lon=slice(80,150)).mean('lat').mean('lon').mean('lev')
...
#ts_anom=ts_4xCO2
east=ts_anom.sel(lat=slice(-5,5),lon=slice(180,280)).mean('lat').mean('lon')
west=ts_anom.sel(lat=slice(-5,5),lon=slice(80,150)).mean('lat').mean('lon')
...
ts_anom=mylist['ts']
east=mylist_control.sel(lat=slice(-5,5),lon=slice(180,280)).mean('lat').mean('lon')
west=mylist_control.sel(lat=slice(-5,5),lon=slice(80,150)).mean('lat').mean('lon')
gradC=west-east
east=ts_anom.sel(lat=slice(-5,5),lon=slice(180,280)).mean('lat').mean('lon')
west=ts_anom.sel(lat=slice(-5,5),lon=slice(80,150)).mean('lat').mean('lon')
grad0=west-east
...
ax[0].bar(x - width, abrupt1_hist, width, yerr=std1_hist, error_kw=dict(lw=5),label='year 2000-2014 minus 1950-1970')
#ax[1].bar(x2 - width, abrupt1_ghg, width, yerr=std1_ghg, label='year 2000-2014 minus 1950-1970, GHGonly')
ax[0].bar(x15, abrupt1_ssp3, width, yerr=std1_ssp3, error_kw=dict(lw=5), label='year 2015-2035, ssp370', hatch="\\")
ax[0].bar(x15 + width, abrupt2_ssp3, width, yerr=std1_ssp3, error_kw=dict(lw=5), label='year 2080-2100, ssp370', hatch="/")
ax[0].bar(x3, obs1, width, label='year 2000-2014 minus 1950-1970, obs', hatch="x")


# Add some text for labels, title and custom x-axis tick labels, etc.
ax[0].set_ylabel('$\Delta$ T ($^o$C)')
ax[0].set_title('Pacific zonal SST gradient change, historical and future projections',fontsize=53)
